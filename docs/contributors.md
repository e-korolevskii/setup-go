...
- cPlease check that no one else has already created a pull request with these or similar changes
...
Unit tests are in the `__tests__` folder, and end-to-end tests are in the `workflows` folder (in the particular file [versions.yml](https://github.com/actions/setup-go/blob/main/.github/workflows/versions.yml)).
